...
class Encoder_denoise(nn.Module):
    ...
    def forward(self, x, temb, return_features=False):
        x1 = x
...
class DenoiseModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t, image=None, return_features=False, return_encoder_features=False):
        x, temb = self.embedding_diffusion(x, t=t, image=image)
        if return_features or return_encoder_features:
            features, temb = self.encoder(x, temb, return_features=True)
            out = self.decoder(*features, temb)
            if return_encoder_features:
                return out, features  # features: [x1, x2, x3, x4, x5]
            else:
                return out, features
        else:
...
class DiffVNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, dropout_rate=0.5):
        ...
        # === 蒸馏用通道对齐层 ===
        # self.proj_for_distill = nn.Conv3d(n_classes, 16, kernel_size=1)
        # student_channels = [32, 64, 128, 256, 512]
        # teacher_channels = [16, 32, 64, 128]

        # # 注意teacher只有4层
        # self.proj_feats_for_distill = nn.ModuleList([
        #     nn.Conv3d(s, t, kernel_size=1)
        #     for s, t in zip(student_channels[:len(teacher_channels)], teacher_channels)
        # ])

    def forward(self, image=None, x=None, pred_type=None, step=None, return_features=False, return_encoder_features=False):
        if pred_type == "q_sample":
            ...
            return self.denoise_model.decoder(x1, x2, x3, x4, x5, temb), None 
