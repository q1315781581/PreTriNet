...
def batch_label_similarity(pred_label, gt_label, num_classes, ignore_background=True):
    """
    Compute per-class Dice and pixel accuracy between predicted labels and GT labels.
    Args:
        pred_label: torch.Tensor [B, 1, D, H, W] - integer labels
        gt_label:   torch.Tensor [B, 1, D, H, W] - integer labels
        num_classes: int - total number of classes
        ignore_background: bool - if True, skip class 0 when reporting
    Returns:
        dice_per_class: List[List[float]] - per sample per class Dice
        acc_per_sample: List[float] - pixel accuracy per sample
    """
    ...
    for b in range(B):
        ...
        dice_per_class.append(sample_dice)
        acc_per_sample.append(acc.item())
    
    return dice_per_class, acc_per_sample